...
def bagging_iteration():
    no_of_iterations = [10, 50, 100]
    bagging_prediction_for_10 = []
    bagging_prediction_for_50 = []
    bagging_prediction_for_100 = []
    ...
    for i in no_of_iterations:
        ...
        for j in range(i):
            avg_prediction = test()
            avg_prediction = avg_prediction[0]
            #print("avg_prediction",len(avg_prediction))
            bagging_prediction.append(avg_prediction)
        ...
        if(i == 10):
             ...
             avg_prediction_for_10 = calculate_avg_of_prediction(bagging_prediction)
        if(i == 50):
            temp = []
            avg_prediction_for_50 = []
            bagging_prediction_for_50 = bagging_prediction
            myshape = str(np.array(bagging_prediction_for_50).shape)
            part1, part2 = myshape.split(', ')
            part1 = part1[1:]  # now is '50'
            part2 = part2[:-1]
            # print("part1",part1)
            ...
            for j in range(int(part2)):
                ...
                ins = []
                for i in range(int(part1)):
                    ins.append(bagging_prediction_for_50[i][j])
                # print("ins",ins)
                val.append(np.argmax(np.bincount(ins)))
                temp.append(ins)

            avg_prediction_for_50.append(val)
        if(i ==100):
            temp = []
            avg_prediction_for_100 = calculate_avg_of_prediction(bagging_prediction)



    print(" avg bagging_prediction_for_10",avg_prediction_for_10)
    print("accuracy for 10 iteration", calculate_accuracy(y_test, avg_prediction_for_10))
    print("error rate for 10 iteration",get_error_rate(y_test,avg_prediction_for_10) )
    print("avg bagging_prediction_for_50", avg_prediction_for_50)
    print("error rate for 50 iteration", get_error_rate(y_test, avg_prediction_for_50))
    print("accuracy for 50 iterations",calculate_accuracy(y_test,avg_prediction_for_50))
    print("avg_bagging_prediction_for_100", avg_prediction_for_100)
    print("error rate for 100 iteration", get_error_rate(y_test, avg_prediction_for_100))
    print("accuracy for 100 iterations", calculate_accuracy(y_test, avg_prediction_for_100))
...
def get_error_rate(actual,predicted):
    error = 0
    for i in range(len(actual)):
        if actual[i] != predicted[0][i]:
            error += 1
