...
def iteration_25():
    for t in tqdm(range(1, 26)):
        ...
        training_error.append(np.sum(s_train_pred != y_train) / y_train.shape[0])
        testing_error.append(np.sum(s_test_pred != y_test) / y_test.shape[0])
    print("training_error for 25 iteration is ", training_error)
    print("testing_error for 25  iteration is ", testing_error)

    plt.figure()
    #plotting the training error and testing error for values 1 to 25
    plt.plot(range(1, 26), training_error, label="Training error")
    plt.plot(range(1, 26), testing_error, label="Testing error")
    plt.title("Training and Testing error of boost(t) for t = 1, 2, ... 25")
    plt.legend()
    plt.show()
...
print("training_error for 50 iteration is ",training_error1)
print("testing_error for 50 iteration is ",testing_error1)
#plotting the training error and testing error for values 1 to 50
plt.figure()
plt.plot(range(1, 51), training_error1, label="Training error")
plt.plot(range(1, 51), testing_error1, label="Testing error")
plt.title("Training and testing error of boost(t) for values t = 1, 2, ... 50")
plt.legend()
plt.show()
...
for t in tqdm(range(1, 11)):
   # print("the value of t is ",t)
    ...
    s_train_pred = np.sign(tot_train)
    s_test_pred = np.sign(tot_test)
    #appending to training error
    testing_error2.append(np.sum(s_test_pred != y_test) / y_test.shape[0])
    #appending the result to testing error
    training_error2.append(np.sum(s_train_pred != y_train) / y_train.shape[0])

print("training_error for 10 iteration is ",training_error2)
print("testing_error for 10 iteration is ",testing_error2)

plt.figure()
plt.plot(range(1, 11), training_error2, label="Training error")
plt.plot(range(1, 11), testing_error2, label="Testing error")
plt.title("Training and Testing error of boost for values  t = 1, 2, ... 10")
plt.legend()
plt.show()
